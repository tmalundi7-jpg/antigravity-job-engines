            ...
            if choice == "1":
                ...
                try:
                    ...
                    for i, ta in enumerate(textareas):
                        ...
                        ai_prompt = f"""You are an expert career coach filling out a UK job application form.
CRITICAL RULE: DO NOT INVENT EXPERIENCE. Use ONLY the facts from the candidate's CV. Write in the first person ('I').

JOB DESCRIPTION CONTEXT (What the employer wants):
{job_description}

CANDIDATE CV:
{cv_text}

TEXT BOX LABEL: {question_text}
SURROUNDING FORM CONTEXT (To help you understand what this box is for):
{question_context}

INSTRUCTIONS:
1. Analyse the SURROUNDING FORM CONTEXT. If this text box is part of a "Work Experience" or "Employment History" entry (e.g. asking for "Role Description" or "Duties"), identify exactly WHICH job from the CV is being asked about (look at the Job Title/Company in the context). Write a factual, succinct 100-word summary of the candidate's duties and achievements for THAT SPECIFIC JOB ONLY from the CV. Do NOT write a generic personal statement.
2. If the question is asking for a general Personal Statement or "Why are you suitable for this role?", write a compelling 500-word statement matching the CV to the job description.
3. If it asks for a Behaviour or Competency (e.g. 'Leadership'), write a 250-word STAR example from the CV.
4. Output ONLY the exact text to be pasted into the box. No pleasantries, no intro/outro, no markdown formatting. Just the raw answer.
"""
