...
def get_real_jobs_from_ddgs(portal: dict, include_entry: bool) -> list[dict]:
    ...
    query_fin = f"site:{domain} (software OR developer OR data OR cyber OR IT OR technology)"
    if include_entry:
        query_fin += " (junior OR graduate OR entry OR trainee OR apprentice OR support)"
        
    # SEARCH 2: Generic Roles
    query_gen = f'site:{domain} (helpdesk OR "1st line" OR desktop OR infrastructure OR analyst)'
    ...
    try:
        ...
        results_fin = DDGS().text(query_fin, max_results=10)
        ...
        results_gen = DDGS().text(query_gen, max_results=10)
